...
public class Contest extends BaseEntity{
    ...
    // Complete the validateQuestionList method to verify if all the questions have the same 
    //level and are equal to contest level.
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        ...
        if(qList==null || qList.isEmpty()){
            throw new InvalidContestException("Question list is emty or null");
        }

        for(Question question:qList){
            if(!question.getLevel().equals(contestLevel)){
                throw new InvalidContestException("Question level do not match with contestLevel");
            }
        }
    }
